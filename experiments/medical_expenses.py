...
df = pd.read_csv("../datasets/health_insurance.csv")  # .drop(['region'], axis=1)
scaler = MinMaxScaler()
columns = df.columns
...
def my_func(selects):
    ...
    for n in range(num_experiments):
        ...
        for s in selects:
            ...
            file_name = "results/medical_expenses_results_{}.csv".format(fs_name)
            ...
            for i in range(x_test_help.shape[1]):
                if s != "Normal":
                    num_feature = i + 1
                    fs_name = s
                    weights = np.loadtxt("results/medical_expenses_weights_{}.csv".format(fs_name), delimiter=",")
                ...
                for name in model_name:
                    ...
                    model = None
                    if name == "WEVREG":
                        model = WEVREG(n_neighbors=50, max_iter=50, learning_rate=0.1, optimizer='rprop',
                                            scale=False,
                                            columns=cols)
                    elif name == "WEVREG1":
                        model = WEVREG(n_neighbors=50, max_iter=50, learning_rate=0.1, optimizer='rprop',
                        ...
                        model = WkNN(n_neighbors=50, max_iter=50, learning_rate=0.1, optimizer='rprop', scale=False)
                    elif name == "SVR":
                    ...
                    with open(file_name, 'a', newline='') as file:
                        writer = csv.writer(file)
                        writer.writerow(
                            [name, n, num_feature, train_time, pred_time, rmse_train, mae_train, mape_train, r2_train,
                             rmse_val, mae_val,
                             mape_val, r2_val])
    if s == "Normal":
        ...
        np.savetxt("results/medical_expenses_weights_{}.csv".format("WEVREG"), weights_wevreg, delimiter=",")
        np.savetxt("results/medical_expenses_weights_{}.csv".format("WEVREG1"), weights_wevreg1, delimiter=",")
        np.savetxt("results/medical_expenses_weights_{}.csv".format("GB"), weights_gb, delimiter=",")
        np.savetxt("results/medical_expenses_weights_{}.csv".format("SVR"), weights_svr, delimiter=",")
        np.savetxt("results/medical_expenses_weights_{}.csv".format("RF"), weights_rf, delimiter=",")
        np.savetxt("results/medical_expenses_weights_{}.csv".format("RT"), weights_rt, delimiter=",")
        np.savetxt("results/medical_expenses_weights_{}.csv".format("WkNN"), weights_wknn, delimiter=",")
