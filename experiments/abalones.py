...
df = pd.read_csv("datasets/abalone.data")  # .drop(['region'], axis=1)
...
def my_func(selects):
    ...
    for n in range(num_experiments):
        ...
        for s in selects:
            ...
            file_name = "results/abalones_results_{}.csv".format(fs_name)
            my_file = Path(file_name)
            if not my_file.is_file():
                with open(file_name, 'w', newline='') as file:
                    writer = csv.writer(file)
                    writer.writerow(
                        ["model", "num_experiment", "num_feature", "train_time", "validation_time", "train_rmse",
                         "train_mae",
                         "train_mape",
            ...
            for i in range(x_test_help.shape[1]):
                if s != "Normal":
                    num_feature = i + 1
                    fs_name = s
                    weights = np.loadtxt("results/abalones_weights_{}.csv".format(fs_name), delimiter=",")
                ...
                for name in model_name:
                    try:
                        model = None
                        stdout.write("\rFS: {}, Model: {}, Num_experiment: {}            \r".format(s, name, n))
                        stdout.flush()
                        stdout.write("\n")
                        if name == "WEVREG":
                            model = WEVREG(n_neighbors=30, max_iter=100, learning_rate=0.1, optimizer='rprop',
                                           scale=False,
                                           columns=cols)
                        elif name == "WEVREG1":
                            model = WEVREG(n_neighbors=30, max_iter=100, learning_rate=0.1, optimizer='rprop',
                            ...
                            model = WkNN(n_neighbors=20, max_iter=50, learning_rate=0.1, optimizer='rprop',
                                             scale=False)
                        elif name == "SVR":
                            model = LinearSVR(C=1000.0, max_iter=1000, dual=False, loss='squared_epsilon_insensitive',
                                              random_state=int(time.time()))
                        elif name == "RT":
                            model = DecisionTreeRegressor()
                        elif name == "KNN":
                            model = KNeighborsRegressor(n_neighbors=30, weights='distance', n_jobs=9)
                        elif name == "RF":
                            model = RandomForestRegressor(n_estimators=100, max_depth=4)
                        elif name == "ANN":
                            model = MLPRegressor(hidden_layer_sizes=(50, 25, 10), max_iter=1500)
                        elif name == "GLM":
                            model = None
                        elif name == "LR":
                            ...
                            model = RGS(n_neighbors=30, epochs=100, learning_rate=0.00001)
                        ...
                        with open(file_name, 'a', newline='') as file:
                            writer = csv.writer(file)
                            writer.writerow(
                                [name, n, num_feature, train_time, pred_time, rmse_train, mae_train, mape_train,
                                 r2_train, rmse_val, mae_val,
                                 mape_val, r2_val])
    ...
    if s == "Normal":
        ...
        np.savetxt("results/abalones_weights_{}.csv".format("WEVREG"), weights_wevreg, delimiter=",")
        np.savetxt("results/abalones_weights_{}.csv".format("WEVREG1"), weights_wevreg1, delimiter=",")
        np.savetxt("results/abalones_weights_{}.csv".format("GB"), weights_gb, delimiter=",")
        np.savetxt("results/abalones_weights_{}.csv".format("SVR"), weights_svr, delimiter=",")
        np.savetxt("results/abalones_weights_{}.csv".format("RF"), weights_rf, delimiter=",")
        np.savetxt("results/abalones_weights_{}.csv".format("RT"), weights_rt, delimiter=",")
        np.savetxt("results/abalones_weights_{}.csv".format("WkNN"), weights_wknn, delimiter=",")
