...
def my_func(selects):
    ...
    for n in range(num_experiments):
            ...
            for s in selects:
                ...
                fs_name = s
                file_name = "wine_quality/wine_quality_results_{}.csv".format(fs_name)
                ...
                for i in range(x_test_help.shape[1]):
                    ...
                    if s != "Normal":
                        num_feature = i + 1
                        weights = np.loadtxt("results/wine_quality_weights_{}.csv".format(fs_name), delimiter=",")
                    ...
                    for name in model_name:
                        ...
                        model = None
                        if name == "WEVREG":
                            model = WEVREG(n_neighbors=100, max_iter=11, learning_rate=0.1, optimizer='rprop', scale=False,
                                                columns=cols)
                        elif name == "WEVREG1":
                            model = WEVREG(n_neighbors=50, max_iter=100, learning_rate=0.1, optimizer='rprop',
                                                scale=False,
                                                columns=cols,
                                                p=1)
                        elif name == "GB":
                        ...
                        with open(file_name, 'a', newline='') as file:
                            writer = csv.writer(file)
                            writer.writerow([name, n, num_feature, train_time, pred_time, rmse_train, mae_train, mape_train, r2_train, rmse_val, mae_val, mape_val, r2_val])
